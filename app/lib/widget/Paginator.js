widget.Paginator = function () {
    ...
    Paginator.prototype._generateButtons = function (from, to, enableDot) {
        ...
        if (enableDot && count - 2 * Paginator.PADDING > Paginator.THRESHOLD) {
            ...
            html += "<button class=\"btn btn-secondary\" type=\"button\"><span>...</span></button>"; //dot
            ...
            for (var i = Math.max(0, from); i < Math.min(to, this.totalPages); i ++) {
                ...
                if (i == this.currentPage) {
                    html += " class=\"btn btn-secondary active\"";
                } else {
    ...
    Paginator.prototype._updateButtonUI = function () {
        ...
        html += "<button type=\"button\" class=\"btn btn-secondary\" role=\"prev\"";
        if (this.currentPage == 0) {
            html += " disabled=\"true\"";
        }
        ...
        html += "<button type=\"button\" class=\"btn btn-secondary\" role=\"next\"";
        if (this.currentPage == this.totalPages - 1) {
            html += " disabled=\"true\"";
        }
