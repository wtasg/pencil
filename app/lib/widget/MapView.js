    ...
    MapView.prototype.setAreaMap = function(map, areaId, showZoneDetail) {
        ...
        $areaService.getZonesInArea(_Long(areaId), function(result) {
            ...
            for (var i = 0; i < renderList.length; i++) {
                ...
                content += "</ul></div>"
                if (showZoneDetail) {
                    content += "<div class=\"EditContainer\"><button type=\"button\" onclick=\"new GenericZoneEditorDialog().open(" + zone.id + ", false);\"" + " class=\"btn btn-secondary zone-edit-icon\"" +
...
//        + "<img class=\"BusinessStatusIcon\" src=\"" + CONTEXT_PATH + "" +"\"/>"
            ...
            Dom.registerEvent(objectView.firstChild, "click", function(e) {
                ...
                if (objectView.nearBy && objectView.nearBy.length > 0) {
                    ...
                    var options = {
                            ...
                            + '" type="button" class="btn-close" owner-id="' + objectView._counterId + '" aria-label="' + Messages["close"] + '" onclick="widget.MapView.closePopOver(this)"></button><div class="popover-arrow"></div><h3 class="popover-header"></h3><div class="popover-body"></div></div>',
                            trigger: "manual"
                        };
    ...
    MapView.prototype.ensurePopOverCreated = function (objectView) {
        ...
        objectView._opts = {
                ...
                viewport: null,
                template: '<div class="popover MapViewPopOver" role="tooltip"><button id="' + closeId
                + '" type="button" class="btn-close" owner-id="' + ownerId + '" aria-label="' + Messages["close"] + '" onclick="widget.MapView.closePopOver(this)"></button><div class="popover-arrow"></div><h3 class="popover-header" id="' + titleId +'"></h3><div class="popover-body"></div></div>',
                trigger: "manual"
            };
...
//        }
    ...
    MapView.prototype.addAsset = function(asset, location, callback, onFocus, disableDetailButton) {
        ...
        if (!disableDetailButton) {
            content += "<div class=\"EditContainer\"><button type=\"button\" class=\"btn btn-secondary popup-edit-icon\" onclick=\"showEditAssetPanel(this)\" title=\"" + Messages["view_asset_details_title"] + "\">" + Messages["asset_detail_button"] + "</button></div>";
        }
